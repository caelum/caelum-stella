...
public class VISATestSolicitacaoDeIntegracaoComComponente {
	...
	public static void before(){
		...
		checkout = new Checkout("123456","Alberto Souza, Av Juracy Magalhaes,Salvador,Bahia,Brasil","Testando checkout",BigDecimal.ONE,new Parcelamento(TipoTransacao.CREDITO_A_VISTA,1),new CartaoCredito("4551870000000183",dataQueExpira,"123"));
	}
	...
	public void testSolicitacaoDeAutorizacaoDePagamento(){
		String telaDeResultadoQueDeveSerApresentadaAoCliente = new VISASolicitacaoAutorizacaoPagamentoViaHttp(checkout).handle();
		System.out.println(telaDeResultadoQueDeveSerApresentadaAoCliente);
	}
		
	public void testSolicitacaoDeCaptura(){
		String retorno = new VISASolicitacaoConferenciaViaHttp(checkout.getTid(Calendar.getInstance(),gatewaysConf.getNumeroDeFiliacaoDoVisa()),VISATipoSolicitacaoIntegracao.CAPTURA).handle();
		System.out.println(retorno);
	}
	
	public void testSolicitacaoDeCancelamento(){
		String retorno = new VISASolicitacaoConferenciaViaHttp(checkout.getTid(Calendar.getInstance(),gatewaysConf.getNumeroDeFiliacaoDoVisa()),VISATipoSolicitacaoIntegracao.CANCELAMENTO).handle();
		System.out.println(retorno);
	}
	
	public void testSolicitacaoDeConsulta(){
		String retorno = new VISASolicitacaoConferenciaViaHttp(checkout.getTid(Calendar.getInstance(),gatewaysConf.getNumeroDeFiliacaoDoVisa()),VISATipoSolicitacaoIntegracao.CONSULTA).handle();
		System.out.println(retorno);
	}	
