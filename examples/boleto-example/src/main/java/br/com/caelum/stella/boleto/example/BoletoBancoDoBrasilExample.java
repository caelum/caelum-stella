...
public class BoletoBancoDoBrasilExample {
	public static void main(String[] args) {
		...
		Emissor emissor = Emissor.newEmissor().withCedente("Fulano de Tal")
				.withAgencia("2345").withDvAgencia("6").withContaCorrente(12345).withNumConvenio("1234567")
                .withDvContaCorrete("1").withCarteira("22").withNossoNumero("0009050987");

		Sacado sacado = Sacado.newSacado().withNome("Fulano da Silva").withCpf("111.222.333-12")
                .withEndereco("Av dos testes, 111 apto 333").withBairro("Bairro Teste")
                .withCep("01234-111").withCidade("São Paulo").withUf("SP");
		...
		Boleto boleto = Boleto.newBoleto().withBanco(banco).withDatas(datas)
                ...
                .withEmissor(emissor).withSacado(sacado).withValorBoleto("200.00").withNoDocumento("1234")
                .withInstrucoes("instrucao 1", "instrucao 2","instrucao 3", "instrucao 4", "instrucao 5")
				.withLocaisDePagamento("local 1", "local 2").withNoDocumento("4343");
