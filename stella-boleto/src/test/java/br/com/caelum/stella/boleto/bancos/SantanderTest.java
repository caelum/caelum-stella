...
public class SantanderTest {
	...
	@Before
	public void setUp() {
		Datas datas = Datas.newDatas().withDocumento(27, 12, 2012).withProcessamento(27, 12, 2012)
				.withVencimento(31, 12, 2012);

		emissor = Emissor.newEmissor().withCedente("Fulano de Tal").withAgencia(4001).withDigitoAgencia('3')
				.withContaCorrente(1216).withDigitoContaCorrente('8').withNumeroConvenio(1207113).withCarteira(102)
		...
		boleto = Boleto.newBoleto().withBanco(banco)
				.withDatas(datas)
				// .withDescricoes("descricao 1", "descricao 2", "descricao 3",
				// "descricao 4", "descricao 5")
				.withEmissor(emissor).withSacado(sacado).withValorBoleto("0.01")
				.withInstrucoes("instrucao 1", "instrucao 2", "instrucao 3", "instrucao 4", "instrucao 5")
				.withLocaisDePagamento("local 1", "local 2").withNumeroDoDocumento("2000");

	}

	@Test
	public void testLinhaDoBancoReal() {
		assertEquals("03390.40016  01216.874691  10800.033721  4  55640000000001",
