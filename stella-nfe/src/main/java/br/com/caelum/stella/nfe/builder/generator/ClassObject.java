...
final public class ClassObject {
	...
	public void addMethod(final String name, final Class<?> parameterType) {
		methodList.add(new MethodObject(name, parameterType.getSimpleName(),
	...
	private String implementationHeader() {
		return getPackageDeclaration() + "public final class "
				+ getInterfaceName() + "Impl implements " + getInterfaceName()
				+ ", " + getObjectCreatorClassName() + " { "
				+ "private final br.com.caelum.stella.nfe.modelo."
				+ source.getSimpleName() + " "
	...
	private String interfaceHeader() {
		return getPackageDeclaration() + "public interface "
				+ getInterfaceName() + " {";
	}
	...
	private String constructor() {
		return "public " + getImplementationName() + " () { this."
				+ asVariable(source.getSimpleName())
				+ " = new br.com.caelum.stella.nfe.modelo."
				+ source.getSimpleName() + "();}";
	}
	...
	private String objectCreator() {
		return "public br.com.caelum.stella.nfe.modelo."
				+ source.getSimpleName() + " getInstance() {" + "return this."
				+ asVariable(source.getSimpleName()) + ";}";
	}

	public String getInterfaceTest() {
		return getPackageDeclaration() + "public final class "
				+ getInterfaceTestName() + " { " + getTestMethods() + " }";
	}
	...
	private String getTestMethods() {
		...
		String signature = "public void test" + getInterfaceName()
				+ "Interface() ";
		...
		methodChain += "; ";
		String assertMethodCall = String.format("assertModelWasFilled(%s);",
				asVariable(getInterfaceName()));
		body += methodChain + assertMethodCall;
		String testMethod = String.format("%s %s { %s }", annotation,
				signature, body);

		String assertWasFilledTestMethod = String.format(
				"private void assertModelWasFilled(%s %s) { "
						+ "new %s (%2$s).assertModelWasFilled();"
						+ "}", getInterfaceName(),
				asVariable(getInterfaceName()), BuilderTestHelper.class.getCanonicalName());
